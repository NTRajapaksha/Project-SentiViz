...
async function analyzeText() {
    ...
    try {
        ...
        if (data.error) {
            ...
            updateTrafficLight(data.sentiment);
        }
    } catch (error) {
        loadingIndicator.classList.add("d-none");
...
function updateVisualizations(data) {
    ...
    updateSentimentMeter(data.score * 100, data.sentiment);
}

function resetVisualizations() {
    updateSentimentMeter(0, null);
    ...
    updateTrafficLight(null);
}

function updateSentimentMeter(score, sentiment) {
    const isDark = isDarkMode();
    let barColor;
    if (sentiment === 'positive') {
        barColor = isDark ? '#4f8' : 'green';
    } else if (sentiment === 'neutral') {
        barColor = isDark ? '#ff7' : '#cc7700';
    } else if (sentiment === 'negative') {
        barColor = isDark ? '#f77' : 'red';
    } else {
        barColor = isDark ? '#555' : '#ccc'; // Default color when no sentiment
    }
    Plotly.newPlot('sentimentMeter', [{
        type: 'indicator',
        mode: 'gauge+number',
        value: score,
        title: { text: 'Sentiment Confidence', font: { color: isDark ? '#fff' : '#000' } },
        number: { suffix: '%', font: { size: 24, color: isDark ? '#fff' : '#000' } },
        gauge: {
            axis: { range: [0, 100], tickwidth: 1, tickcolor: isDark ? '#aaa' : 'darkblue', tickfont: { color: isDark ? '#fff' : '#000' } },
            bar: { color: barColor },
            bgcolor: isDark ? '#333' : 'white',
            borderwidth: 2,
            bordercolor: isDark ? '#777' : 'gray',
            steps: [
                { range: [0, 100], color: isDark ? '#333' : '#eee' }
            ],
            threshold: { line: { color: isDark ? 'white' : 'black', width: 4 }, thickness: 0.75, value: score }
        }
    }], getPlotlyLayout('sentimentMeter'));
}

function updateTrafficLight(sentiment) {
